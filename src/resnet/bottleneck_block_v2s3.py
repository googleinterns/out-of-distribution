...
class BottleneckBlockV2S3(nn.Module):
    bn1: nn.BatchNorm2d
    conv1: nn.Conv2d
    bn2: nn.BatchNorm2d
    conv2: nn.Conv2d
...
def create_bottleneck_stage_v2s3(
    n_blocks: int, in_channels: int, mid_channels: int, out_channels: int, stride: int
) -> nn.Sequential:
    blocks = [
        BottleneckBlockV2S3(in_channels, mid_channels, out_channels, stride) if i == 0
        else BottleneckBlockV2S3(out_channels, mid_channels, out_channels, 1)
