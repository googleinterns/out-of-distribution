...
def create_bottleneck_stage(
    n_blocks: int, in_channels: int, mid_channels: int, out_channels: int, stride: int
) -> nn.Sequential:
    blocks = [
        BottleneckBlock(in_channels, mid_channels, out_channels, stride) if i == 0
        else BottleneckBlock(out_channels, mid_channels, out_channels, 1)
